...
def _extract_hrefs_from_semester_link_elements(semester_link_elements) -> list[str]:
    sem_urls = []
...
def get_semester_urls(webdriver: WebDriver, base_url: str) -> list[str]:
    _navigate_to_base_url(webdriver, base_url)
    semester_link_elements = _fetch_semester_links(webdriver)
    semester_urls = _extract_hrefs_from_semester_link_elements(semester_link_elements)
